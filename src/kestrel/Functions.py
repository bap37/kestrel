...
def DistExponential(x, theta, correlation):
    """
    Exponential importance-weight for batched θ.

    Uses a fixed proposal distribution (tau_proposal=0.5, hard-coded for
    selection-effect reasons) and returns the importance weight
    p_target(x) / p_proposal(x) rather than the raw likelihood.

    x:     Tensor of shape (N,) or (batch_size, N)
    theta: Tensor of shape (batch_size, 1) -> [tau] for each θ

    Returns:
        Tensor of shape (batch_size, N) -> importance weight of each x for each θ
    """
    x = torch.as_tensor(x, dtype=torch.float32)
    theta = torch.as_tensor(theta, dtype=torch.float32)
...
def DistExponential_EVOL(x, theta, correlation):
    """Exponential likelihood with evolution (not yet implemented; returns None)."""
...
def DistLogistic(x, theta, correlation):
    ...
    Models RV as a logistic function of host mass (correlation), with
    Gaussian scatter sigma around the logistic mean.
...
def DistDelta(theta):
    """
    Delta-function parameter: theta is used directly as a fixed value
    rather than being sampled from a continuous distribution.

    theta: Tensor of shape (batch_size, 1) -> point-mass value for each θ

    Returns:
        None (the delta parameter is injected via extra_tensors in the simulator)
    """
    theta = torch.as_tensor(theta, dtype=torch.float32)
