...
@dataclass
class ThetaLayout:
    """Describes how the flat theta vector is partitioned by distribution type.

    Attributes:
        slices:   Maps distribution-type key → slice into the flat theta vector.
        idx:      Maps distribution-type key → list of original param indices.
        counts:   Maps distribution-type key → number of parameters of that type.
...
def build_theta_layout(counts, n_params):
    ...
    Args:
        counts:   Dict mapping distribution-type key → number of parameters.
        n_params: Dict mapping distribution-type key → scalars-per-parameter.

    Returns:
        Dict mapping distribution-type key → slice into the flat theta vector.
    """
...
def build_layout(param_names, dicts):
    """Build a ThetaLayout from the list of parameter names and configuration dicts.

    Iterates over param_names, looks up each parameter's distribution function,
    and assigns it to the appropriate distribution-type bucket.  Handles the
    _HIGH_ suffix (Stepwise split parameters) and Truncated prefix transparently.

    Args:
        param_names: Ordered list of parameter names (as returned by
                     parameter_generation, including _HIGH_ variants).
        dicts:       [function_dict, split_dict, priors_dict, corr_dict]
                     as loaded from the YAML config.

    Returns:
        ThetaLayout dataclass with fully populated slices, idx, counts,
        n_params, and order fields.
    """
...
def make_batched_simulator(layout, df, param_names, parameters_to_condition_on,
    ...
    produces simulated population observables (B, n_target, n_features) by
    importance-resampling from the pre-loaded simulation bank.

    Key steps inside the closure:
      1. Compute per-SN importance weights for each theta using the Dist*
         likelihood functions defined in Functions.py.
      2. Normalise weights and check effective sample size (ESS).
      3. Multinomial-resample n_target SNe from the bank for each theta.
      4. Optionally add a host-mass step (STEP) and grey scatter (SCATTER).
      5. Mark low-ESS or zero-weight draws as NaN.

    Args:
        layout:                    ThetaLayout from build_layout.
        df:                        Simulation-bank DataFrame (already loaded).
        param_names:               List of parameter names from the YAML config.
        parameters_to_condition_on: Ordered list of observable column names.
        dicts:                     [function_dict, split_dict, priors_dict, corr_dict].
        dfdata:                    Observed data DataFrame (sets n_target).
        sub_batch:                 Max theta rows processed in a single GPU call
                                   (controls memory vs. speed trade-off).
        device:                    'cpu' or 'cuda'.
        debug:                     If True, return raw DataFrame rows instead of tensors.
        mixture:                   If True, theta encodes two populations + mixing fraction.

    Returns:
        batched_simulator: Callable[[Tensor], Tensor] with signature
                           (B, n_params) → (B, n_target, n_features).
    """
    function_dict, split_dict, priors_dict, corr_dict = dicts
...
def preprocess_input_distribution(df, cols):
    ...
    Args:
        df:   DataFrame containing the requested columns.
...
def parameter_generation(list_of_parameter_names, dicts):
    """Expand param_names to include _HIGH_ variants for Stepwise-split parameters.

    For each parameter that has a Stepwise split in split_dict, inserts a
    corresponding '<name>_HIGH_<split_col>' entry immediately before the
    base name in the output list.  STEP is excluded from this expansion.

    Args:
        list_of_parameter_names: Base parameter names from the YAML config.
        dicts: [function_dict, split_dict, priors_dict, corr_dict].

    Returns:
        Expanded list of parameter name strings (including _HIGH_ variants).
    """
...
def validate_step(param_names, params_to_avoid):
    """Verify that special parameters (STEP, SCATTER) appear at the end of param_names in the correct order.

    Args:
        param_names:    Full list of parameter names.
        params_to_avoid: Ordered list of special parameter names that must be last.

    Raises:
        ValueError: If the special parameters are not at the end or are out of order.
    """
...
def preprocess_data(parameters_to_condition_on, dfdata):
    """Stack observed data columns into a 2-D tensor ready for the network.

    Args:
        parameters_to_condition_on: Ordered list of observable column names.
        dfdata: Observed data DataFrame.

    Returns:
        Tensor of shape (n_sne, n_features).
    """
...
def load_kestrel(filename):
    """Parse a Kestrel YAML config file into a ready-to-use Python dict.

    Handles the following transformations:
      - Splits whitespace-delimited strings into lists (param_names, Data_File, etc.).
      - Converts prior bound strings (e.g. "(1.5, 4)") to Python tuples via ast.literal_eval.
      - Resolves function name strings (e.g. "DistGaussian") to callable objects.
      - Parses Population_B and Models_Comparison sections when present.

    Args:
        filename: Path to a KESTREL-style YAML file.

    Returns:
        Dict with all keys parsed and ready for use by run_sims.py / run_nre.py.
    """
...
def load_data(simfilename, datfilename):
    ...
    Applies several preprocessing steps:
      - Computes SIM_EBV = SIM_AV / SIM_RV for the simulation bank.
      - Computes the Planck18 distance modulus (MU) for both datasets.
      - Restricts observed data to DES survey (IDSURVEY == 10).
      - Applies a SNN classifier cut (PROB_SNNV19 >= 0.5).
      - Removes SNe with invalid host log-masses.

    Args:
        simfilename: Path to the simulation-bank FITRES file (can be gzipped).
        datfilename: Path to the observed-data FITRES file (can be gzipped).
...
def standardise_data(dft, dfdata, parameters_to_condition_on, param_names):
    ...
    Error columns are log-transformed before standardisation.  Applies
    separate standardisation to simulations and data, and returns the
    mean/std used so that posterior samples can be back-transformed.

    Args:
        dft:                       Simulation-bank DataFrame.
        dfdata:                    Observed-data DataFrame.
        parameters_to_condition_on: Observable column names to standardise.
        param_names:               Input parameter names (also standardised in dft).

    Returns:
        Tuple (dft, dfdata, return_dict) where return_dict maps
        '<col>_data' / '<col>_sim' → [mean, std] used for normalisation.
    """
...
def simulate_model(n_sim, n_batch, sims_savename, priors, simulator, inference, device="cpu", batched=True):
    ...
    (theta, x) pairs incrementally to avoid holding the full dataset in
    memory.  After all batches are done, returns the valid (non-NaN) theta
    samples and a fresh prior draw for diagnostic plotting.

    Args:
        n_sim:          Total number of simulations to generate.
        n_batch:        Number of theta draws per simulator call.
        sims_savename:  Path for the output HDF5 file (created/overwritten).
        priors:         sbi MultipleIndependent prior object.
        simulator:      Batched simulator callable from make_batched_simulator.
        inference:      sbi SNPE inference object (used for shape probing only here).
        device:         'cpu' or 'cuda'.
        batched:        If True, pass the full batch to the simulator at once;
                        otherwise loop over individual theta rows.

    Returns:
        Tuple (theta_valid, p_vals) where theta_valid contains only draws that
        produced non-NaN simulations, and p_vals is a fresh prior sample of
        size n_sim for plotting against theta_valid.
    """
    theta_tot = []
    mask_tot = []
...
def train_spne(prior, x):
    """Sequential Neural Posterior Estimation training loop (prototype / unused).

    Runs SNPE for num_rounds rounds with num_simulations per round,
    updating the proposal distribution after each round.

    Args:
        prior: sbi prior object.
        x:     Observed data tensor used to set the default conditioning point.
...
def add_distance(mcmc, df_tensor):
    ...
    Args:
        mcmc:      A Pyro MCMC object whose get_samples() returns posterior draws.
        df_tensor: Dict of tensors with keys 'x1', 'c', 'mB', 'MU'.
...
def start_distance(NUM_WARMUP = 50, NUM_SAMPLES = 150, NUM_CHAINS = 1):
    ...
    Args:
        NUM_WARMUP:  Number of HMC warm-up steps.
...
def distancinator(x0_obs, x0_err, x1_obs, x1_err, c_obs, c_err, dist_mod):
    ...
    Samples nuisance parameters alpha, beta, M, sigma_int and per-SN
    latent fluxes, then conditions on the observed mB and distance modulus.

    Args:
        x0_obs, x0_err: Observed flux amplitude and its uncertainty.
        x1_obs, x1_err: Observed stretch and its uncertainty.
        c_obs, c_err:   Observed colour and its uncertainty.
        dist_mod:       Cosmological distance modulus for each SN.
    """
...
def prior_generator(param_names, dicts, device='cpu'):
    ...
    result in sbi's MultipleIndependent for use with SNPE.
