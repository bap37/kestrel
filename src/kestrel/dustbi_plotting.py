...
def plot_surviving_priors(theta, priors, labels, figname):
    """Plot histograms comparing surviving (valid) prior samples to the full prior.

    After simulation, some theta draws produce NaN outputs and are rejected.
    This plot shows how the valid subset differs from the full prior draw,
    revealing any selection effects introduced by the simulator.

    Args:
        theta:   Tensor of shape (n_valid, n_params) — parameter samples that
                 produced non-NaN simulations.
        priors:  Tensor of shape (n_full, n_params) — the full prior draw.
        labels:  List of LaTeX strings for each parameter (from unspool_labels).
        figname: Output PDF path.
    """
...
def plot_tarp(ecp, alpha, savename):
    """Plot a TARP (Test of Accuracy with Random Points) coverage curve.

    Args:
        ecp:      Expected coverage probability array (y-axis).
        alpha:    Credibility level array (x-axis).
        savename: Output path for the saved figure.
...
def plot_loss(inference, savename):
    """Plot training and validation loss curves from an SNPE inference object.

    Args:
        inference: A trained sbi SNPE object whose _summary dict contains
                   'training_loss' and 'validation_loss' lists.
        savename:  Output path for the saved figure.
    """
